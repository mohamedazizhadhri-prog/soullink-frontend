...
export class FriendsService {
    async sendRequest(senderId, receiverId) {
        ...
        if (existing) {
            ...
            if (existing.status === 'PENDING')
                throw new AppError(400, 'A friend request is already pending');
    ...
    async respondRequest(userId, friendshipId, action) {
    ...
    async listFriends(userId, limit = 50, cursor) {
    ...
    async listPendingRequests(userId, limit = 50, cursor) {
    ...
    async listSentRequests(userId, limit = 50, cursor) {
        const requests = await prisma.friendship.findMany({
            where: {
                senderId: userId,
                status: 'PENDING'
