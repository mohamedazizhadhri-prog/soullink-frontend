...
function getFriendshipStage(trustLevel) {
    if (trustLevel <= 20)
        return "NEW";
    if (trustLevel <= 45)
        return "FAMILIAR";
...
export class MemoryService {
     ...
     */
    async extractFactsAndUpdateMemory(userId, recentMessages) {
...
`;
            ...
            const mergeUnique = (oldArr, newArr) => Array.from(new Set([...(oldArr || []), ...(newArr || [])]));
     ...
     */
    async recordTrustEvent(userId, eventType) {
        try {
            let memory = await prisma.novaUserMemory.findUnique({ where: { userId } });
            if (!memory) {
                memory = await prisma.novaUserMemory.create({
