...
io.use(async (socket, next) => {
    try {
        ...
        try {
            ...
            logger.error(`Socket Auth: JWT verification failed for socket ${socket.id}. Error: ${jwtErr.message}`);
        ...
        try {
            ...
            // Distinguish between actual auth errors and server/db errors
            return next(new Error('Internal server error: Unable to verify user (Database connection issues?)'));
        }
    }
    catch (err) {
        logger.error(`Socket Auth: Unexpected error for socket ${socket.id}: ${err.message}`);
...
io.on('connection', async (socket) => {
    ...
    if (!userSockets.has(user.id)) {
        ...
        try {
            ...
            logger.error(`Presence Online Error for ${user.id}: ${err.message}`);
        }
    }
        ...
        if (userSet) {
            ...
            if (userSet.size === 0) {
                ...
                try {
                    ...
                    logger.error(`Presence Offline Error for ${user.id}: ${err.message}`);
                }
            }
        }
    });
    ...
    socket.on('community:leave', (serverId) => {
...
const resetUserPresence = async () => {
    try {
        ...
        logger.error(`Presence Reset Error: ${err.message}`);
    }
};
resetUserPresence();
export { server, io };
