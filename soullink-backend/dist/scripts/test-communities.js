...
async function run() {
    ...
    try {
        ...
        if (community.name !== 'Test Kingdom')
            throw new Error('Wrong name');
        if (community.channels.length === 0)
            throw new Error('Default channels not created');
        console.log('   Pass (Invite Code: ' + community.inviteCode + ')');
        // 2. Join via Invite
        console.log('2. Joining via invite code...');
        await service.joinByInvite(user2.id, community.inviteCode);
        const detail = await service.getCommunity(community.id);
        if (detail._count.members !== 2)
            throw new Error('Member count should be 2');
        console.log('   Pass');
        // 3. Send Message
        ...
        if (message.content !== 'Hello Test World!')
        ...
        if (messages.length === 0 || messages[0].content !== 'Hello Test World!')
