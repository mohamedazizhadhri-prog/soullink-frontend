...
export class ChatController {
    ...
    async sendMessage(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            const { friendId } = req.params;
            let { content, type, replyToId } = req.body;
    ...
    async togglePin(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            ...
            const message = await chatService.togglePin(req.user!.id, friendId as string, messageId as string);
    ...
    async getPinnedMessages(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            const { friendId } = req.params;
            const messages = await chatService.getPinnedMessages(req.user!.id, friendId as string);
