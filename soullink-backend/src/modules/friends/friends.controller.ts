...
export class FriendsController {
    ...
    async respondRequestByUser(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            const { action } = req.body;
            const senderId = req.params.userId as string;
            const receiverId = req.user!.id;
            
            const friendship = await friendsService.respondToRequestByUser(receiverId, senderId, action);
