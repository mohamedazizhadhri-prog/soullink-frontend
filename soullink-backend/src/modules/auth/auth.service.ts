...
export class AuthService {
    ...
    async enrollFace(userId: string, imageUrl: string) {
        ...
        return await prisma.user.update({
            ...
            data: {
                avatarUrl: imageUrl,
                status: (user.emailVerified || user.phoneVerified) ? 'ACTIVE' : 'PENDING_VERIFICATION',
                faceVerified: true,
            },
        });
    }
    ...
    async forgotPassword(identifier: string) {
        ...
        if (!user) {
            throw new AppError(404, 'No account found with this email or phone number.');
        }
    ...
    async logout(token: string) {
        // 1) Immediately evict from the in-memory cache so the token
        //    is invalid right away, even before the DB write resolves.
        invalidateAuthCache(token);

        // 2) Persist to DB blacklist to cover server restarts / other instances
