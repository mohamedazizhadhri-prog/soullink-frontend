...
async function run() {
    ...
    try {
        ...
        if (pending.length === 0 || pending[0].sender.id !== userA.id) throw new Error('Pending request not found');
        console.log('   Pass');
        ...
        if (friendsA.length === 0 || friendsA[0].id !== userB.id) throw new Error('Friend not found in list');
        console.log('   Pass');

        console.log('5. Verifying friend list for User B...');
        const friendsB = await service.listFriends(userB.id);
        if (friendsB.length === 0 || friendsB[0].id !== userA.id) throw new Error('Friend not found in list');
        console.log('   Pass');
        ...
        try {
            ...
            if (e.message !== 'You cannot send a friend request to yourself') throw e;
            console.log('   Pass');
        }
        ...
        if (friendsAfter.length !== 0) throw new Error('Friend should be removed');
        console.log('   Pass');
