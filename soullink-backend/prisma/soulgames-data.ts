import { SceneSeed } from './seed-soulgames';

export const officialSoulGames = [
    // ═══════════════════════════════════════════
    // EPISODE 1: "The Inner Crossroads"
    // ═══════════════════════════════════════════
    {
        slug: 'inner-crossroads',
        title: 'The Inner Crossroads',
        description: 'Episode 1 - Big Five Personality Assessment',
        theme: 'personality',
        color: '#6366f1',
        icon: '🌌',
        mandatory: true,
        episode: 1,
        orderIndex: 1,
        scenes: [
            {
                sceneOrder: 1,
                text: "Worry about things",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 2,
                text: "Make friends easily",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 3,
                text: "Have a vivid imagination",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 4,
                text: "Trust others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 5,
                text: "Complete tasks successfully",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 6,
                text: "Get angry easily",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 7,
                text: "Love large parties",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 8,
                text: "Believe in the importance of art",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 9,
                text: "Use others for my own ends",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 10,
                text: "Like to tidy up",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 11,
                text: "Often feel blue",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 12,
                text: "Take charge",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 13,
                text: "Experience my emotions intensely",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 14,
                text: "Love to help others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 15,
                text: "Keep my promises",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 16,
                text: "Find it difficult to approach others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 17,
                text: "Am always busy",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 18,
                text: "Prefer variety to routine",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 19,
                text: "Love a good fight",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 20,
                text: "Work hard",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#6366f1',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
        ] as SceneSeed[]
    },

    // ═══════════════════════════════════════════
    // EPISODE 2: "The Mirror Lake"
    // ═══════════════════════════════════════════
    {
        slug: 'mirror-lake',
        title: 'The Mirror Lake',
        description: 'Episode 2 - Big Five Personality Assessment',
        theme: 'personality',
        color: '#10b981',
        icon: '🪞',
        mandatory: false,
        episode: 2,
        orderIndex: 2,
        scenes: [
            {
                sceneOrder: 1,
                text: "Go on binges",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 2,
                text: "Love excitement",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 3,
                text: "Love to read challenging material",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 4,
                text: "Believe that I am better than others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 5,
                text: "Am always prepared",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 6,
                text: "Panic easily",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 7,
                text: "Radiate joy",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 8,
                text: "Tend to vote for liberal political candidates",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 9,
                text: "Sympathize with the homeless",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 10,
                text: "Jump into things without thinking",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 11,
                text: "Fear for the worst",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 12,
                text: "Feel comfortable around people",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 13,
                text: "Enjoy wild flights of fantasy",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 14,
                text: "Believe that others have good intentions",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 15,
                text: "Excel in what I do",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 16,
                text: "Get irritated easily",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 17,
                text: "Talk to a lot of different people at parties",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 18,
                text: "See beauty in things that others might not notice",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 19,
                text: "Cheat to get ahead",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 20,
                text: "Often forget to put things back in their proper place",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#10b981',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
        ] as SceneSeed[]
    },

    // ═══════════════════════════════════════════
    // EPISODE 3: "The Shadow Garden"
    // ═══════════════════════════════════════════
    {
        slug: 'shadow-garden',
        title: 'The Shadow Garden',
        description: 'Episode 3 - Big Five Personality Assessment',
        theme: 'personality',
        color: '#ec4899',
        icon: '🌑',
        mandatory: false,
        episode: 3,
        orderIndex: 3,
        scenes: [
            {
                sceneOrder: 1,
                text: "Dislike myself",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 2,
                text: "Try to lead others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 3,
                text: "Feel others' emotions",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 4,
                text: "Am concerned about others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 5,
                text: "Tell the truth",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 6,
                text: "Am afraid to draw attention to myself",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 7,
                text: "Am always on the go",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 8,
                text: "Prefer to stick with things that I know",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 9,
                text: "Yell at people",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 10,
                text: "Do more than what's expected of me",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 11,
                text: "Rarely overindulge",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 12,
                text: "Seek adventure",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 13,
                text: "Avoid philosophical discussions",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 14,
                text: "Think highly of myself",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 15,
                text: "Carry out my plans",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 16,
                text: "Become overwhelmed by events",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 17,
                text: "Have a lot of fun",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 18,
                text: "Believe that there is no absolute right and wrong",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 19,
                text: "Feel sympathy for those who are worse off than myself",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 20,
                text: "Make rash decisions",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#ec4899',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
        ] as SceneSeed[]
    },

    // ═══════════════════════════════════════════
    // EPISODE 4: "The Echo Chamber"
    // ═══════════════════════════════════════════
    {
        slug: 'echo-chamber',
        title: 'The Echo Chamber',
        description: 'Episode 4 - Big Five Personality Assessment',
        theme: 'personality',
        color: '#8b5cf6',
        icon: '💎',
        mandatory: false,
        episode: 4,
        orderIndex: 4,
        scenes: [
            {
                sceneOrder: 1,
                text: "Am afraid of many things",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 2,
                text: "Avoid contacts with others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 3,
                text: "Love to daydream",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 4,
                text: "Trust what people say",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 5,
                text: "Handle tasks smoothly",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 6,
                text: "Lose my temper",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 7,
                text: "Prefer to be alone",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 8,
                text: "Do not like poetry",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 9,
                text: "Take advantage of others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 10,
                text: "Leave a mess in my room",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 11,
                text: "Am often down in the dumps",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 12,
                text: "Take control of things",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 13,
                text: "Rarely notice my emotional reactions",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 14,
                text: "Am indifferent to the feelings of others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 15,
                text: "Break rules",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 16,
                text: "Only feel comfortable with friends",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 17,
                text: "Do a lot in my spare time",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 18,
                text: "Dislike changes",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 19,
                text: "Insult people",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 20,
                text: "Do just enough work to get by",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#8b5cf6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
        ] as SceneSeed[]
    },

    // ═══════════════════════════════════════════
    // EPISODE 5: "The Cloud City"
    // ═══════════════════════════════════════════
    {
        slug: 'cloud-city',
        title: 'The Cloud City',
        description: 'Episode 5 - Big Five Personality Assessment',
        theme: 'personality',
        color: '#3b82f6',
        icon: '☁️',
        mandatory: false,
        episode: 5,
        orderIndex: 5,
        scenes: [
            {
                sceneOrder: 1,
                text: "Easily resist temptations",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 2,
                text: "Enjoy being reckless",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 3,
                text: "Have difficulty understanding abstract ideas",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 4,
                text: "Have a high opinion of myself",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 5,
                text: "Waste my time",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 6,
                text: "Feel that I'm unable to deal with things",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 7,
                text: "Love life",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 8,
                text: "Tend to vote for conservative political candidates",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 9,
                text: "Am not interested in other people's problems",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 10,
                text: "Rush into things",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 11,
                text: "Get stressed out easily",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 12,
                text: "Keep others at a distance",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 13,
                text: "Like to get lost in thought",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 14,
                text: "Distrust people",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 15,
                text: "Know how to get things done",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 16,
                text: "Am not easily annoyed",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 17,
                text: "Avoid crowds",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 18,
                text: "Do not enjoy going to art museums",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 19,
                text: "Obstruct others' plans",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 20,
                text: "Leave my belongings around",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#3b82f6',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
        ] as SceneSeed[]
    },

    // ═══════════════════════════════════════════
    // EPISODE 6: "The Clockwork Spire"
    // ═══════════════════════════════════════════
    {
        slug: 'clockwork-spire',
        title: 'The Clockwork Spire',
        description: 'Episode 6 - Big Five Personality Assessment',
        theme: 'personality',
        color: '#f59e0b',
        icon: '⚙️',
        mandatory: false,
        episode: 6,
        orderIndex: 6,
        scenes: [
            {
                sceneOrder: 1,
                text: "Feel comfortable with myself",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 2,
                text: "Wait for others to lead the way",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 3,
                text: "Don't understand people who get emotional",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 4,
                text: "Take no time for others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 5,
                text: "Break my promises",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 6,
                text: "Am not bothered by difficult social situations",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 7,
                text: "Like to take it easy",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 8,
                text: "Am attached to conventional ways",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 9,
                text: "Get back at others",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 10,
                text: "Put little time and effort into my work",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 11,
                text: "Am able to control my cravings",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 12,
                text: "Act wild and crazy",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 13,
                text: "Am not interested in theoretical discussions",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 14,
                text: "Boast about my virtues",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 15,
                text: "Have difficulty starting tasks",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 16,
                text: "Remain calm under pressure",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 5,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 4,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 3,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 2,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 1,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 17,
                text: "Look at the bright side of life",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 1,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 2,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 3,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 4,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 5,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 18,
                text: "Believe that we should be tough on crime",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 5,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 4,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 3,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 2,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 1,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 19,
                text: "Try not to think about the needy",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 5,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 4,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 3,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 2,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 0,
                        extraversion: 0,
                        agreeableness: 1,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
            {
                sceneOrder: 20,
                text: "Act without thinking",
                bgImageUrl: null,
                novaComment: "How accurately does this describe your typical behavior?",
                novaMood: "neutral",
                choices: [
                    {
                        orderIndex: 1,
                        text: "Very Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 5,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 2,
                        text: "Moderately Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 4,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 3,
                        text: "Neither Accurate Nor Inaccurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 3,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 4,
                        text: "Moderately Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 2,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                    {
                        orderIndex: 5,
                        text: "Very Accurate",
                        themeColor: '#f59e0b',
                        reflectionPrompt: "Assessment in progress...",
                        openness: 0,
                        conscientiousness: 1,
                        extraversion: 0,
                        agreeableness: 0,
                        neuroticism: 0,
                        valueWeight: 0
                    },
                ]
            },
        ] as SceneSeed[]
    },

];
