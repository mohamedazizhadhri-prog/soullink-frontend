...
/**
 * Moderation Feedback Page — /moderation/feedback
 *
 * Identical feature set to /admin/feedback but scoped to the
 * MODERATOR role. Uses moderationApi.listFeedback / replyToFeedback / closeFeedback.
 */

import React, { useEffect, useState, useCallback } from 'react';
import ModerationSidebar from '@/components/admin/ModerationSidebar';
import { moderationApi } from '@/lib/adminApi';
import styles from '../../admin/feedback/page.module.css';
...
export default function ModerationFeedbackPage() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            const data = await moderationApi.listFeedback(params);
    ...
    const handleReply = async (id: string) => {
        ...
        setSubmitting(true);
        try {
            await moderationApi.replyToFeedback(id, replyText.trim());
            setReplyingTo(null);
            setReplyText('');
            load();
        } catch (e: any) {
    ...
    const handleClose = async (id: string) => {
        ...
        try {
            await moderationApi.closeFeedback(id);
            load();
        } catch (e: any) {
        ...
        <div className={styles.shell}>
            ...
            <main className={styles.main}>
                ...
                <header className={styles.header}>
                    ...
                    {/* Moderator badge — teal instead of purple */}
                    <span style={{
                        display: 'inline-flex', alignItems: 'center',
                        padding: '4px 12px', borderRadius: '999px',
                        fontSize: '11px', fontWeight: 700, letterSpacing: '0.08em',
                        background: 'linear-gradient(135deg, rgba(0,191,255,0.2), rgba(32,178,170,0.1))',
                        border: '1px solid rgba(0,191,255,0.3)', color: '#88DDFF',
                        whiteSpace: 'nowrap',
                    }}>
                        MODERATOR
                    </span>
                                ...
                                <article key={fb.id} className={styles.card}>
                                    ...
                                    {fb.reply && (
                                        <div className={styles.existingReply}>
                                            <span className={styles.replyLabel}>✉️ Reply sent</span>
