...
import eyeballImg from "@/assest/eyeball.jpg";
import chickenGif from "@/assest/chicken.gif";
...
export function NovaAvatar() {
    const {
        ...
        consumedFriendIds, isCinematicMode, setMood, waterLevel, splashTrigger,
        isStormMode, isAscending, setIsAscending, triggerEmote,
        isDead, setIsDead, setChatDisabled, deathPhase, setDeathPhase, addMessage,
        eatenElements, triggerBlush, curiousTarget, setCuriousTarget,
        novaPosition, setNovaPosition, investigationContext, setInvestigationContext,
        homePosition, isHunting, isSpitting, huntTarget, isSingularity,
        isKillingMachine, targetingData, isFiring, recordInteraction,
        isHeartbroken, isMelting, temperature, isSmart, isEgg, setIsEgg
    } = useNova();
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            setMoneyParticles(prev => prev.map(p => {
                // Bounce logic for coins
                let newY = p.y + p.vy;
                let newVy = p.vy;

                if (p.type === 'coin' && newY > 40) { // Hit "bottom" of orb container
                    newY = 40;
                    newVy = -newVy * 0.6; // Bounce with damping
                }

                return {
                    ...p,
                    x: p.x + p.vx + (p.type === 'bill' ? Math.sin(Date.now() / 200) * 0.5 : 0), // Flutter bills
                    y: newY,
                    vy: p.type === 'coin' ? newVy + 0.5 : newVy, // Gravity for coins
                    rotation: p.rotation + (p.type === 'bill' ? 2 : 5)
                };
            }).filter(p => p.y < 100)); // Remove when fallen off screen
        }, 30);
    ...
    useEffect(() => {
        ...
        if (drownTimer >= 150 && !isAscending && !isDead) { // 15s total
            ...
            setTimeout(() => {
                ...
                setIsDead(false); // Nova is no longer "dead", he is reborn
                setDeathPhase(5); // Transformation complete
            }, 4500);

            // Slow ascension starts after 5.5s
            setTimeout(() => {
                setIsAscending(true);
            }, 5500);
        }
    ...
    const eyeDefaultX = useSpring(mouseX, { damping: 25, stiffness: 150 });
    const eyeDefaultY = useSpring(mouseY, { damping: 25, stiffness: 150 });
    ...
    useEffect(() => {
        ...
        const spawnWealthBurst = () => {
            const newParticles = Array.from({ length: 20 }).map((_, i) => ({
                ...
                y: 0,
                type: Math.random() > 0.5 ? 'coin' as const : 'bill' as const,
                rotation: Math.random() * 360,
                vx: (Math.random() - 0.5) * 10,
                vy: (Math.random() - 1) * 10 - 5 // Up and out
            }));
                ...
                investigationTimerRef.current = setTimeout(async () => {
                    ...
                    try {
                        const response = await fetch('/api/groq-chat', {
                            ...
                            body: JSON.stringify({
                                message: `Investigate this ${tagName}: "${text}". Short curious comment (10 words max).`,
                                mood: 'curious'
                            })
                        });
    ...
    const getColors = () => {
        ...
        if (isDead || deathPhase >= 2) return { core: "linear-gradient(135deg, #2d3436 0%, #1e272e 100%)", eye: "#636e72" }; // Charcoal
        ...
        if (isSmart) return { core: "linear-gradient(135deg, #1e3a8a 0%, #0d9488 100%)", eye: "#E0FFFF" }; // Deep blue to teal
        ...
        switch (emote || mood) { // Emote overrides mood colors if active
            ...
            case "surprised": return { core: "linear-gradient(135deg, #4B0082 0%, #7B68EE 50%, #00BFFF 100%)", eye: "#FFFFFF" };
            default:
    ...
    const coreVariants: Variants = {
        ...
        nod: {
            scaleY: [0.9, 1.1, 0.95, 1.05, 0.98, 1],
            transition: {
                duration: 0.9,
                times: [0, 0.2, 0.4, 0.6, 0.8, 1]
            }
    ...
    const handleHatchClick = () => {
        ...
        if (!isEgg) {
            setIsEgg(true);
            // Immediate start for direct interaction
            setTimeout(startHatching, 300);
        } else if (hatchPhase === 'egg') {
            startHatching();
        }
    };


    const activeGesture = isAscending ? "ascending" : (isDead ? "sinking" : (deathPhase === 1 ? "thrashing" : (isBurping ? "burp" : (isSmart ? "smart" : (mood === "happy" ? "happy" : (emote === "nod" || emote === "shake") ? emote : (mood === "laugh" ? "laugh" : (mood === "sad" ? "sad" : (mood === "disgusted" ? "disgusted" : (mood === "bored" ? "bored" : (isSubmerged ? "underwater" : "idle"))))))))));
    const combinedY = smoothFallY;

    const activeEmotion = status === "offline" ? "offline" : (isPacManMode ? "idle" : (isMelting ? "melting" : (isSmart ? "smart" : (mood === "neutral" ? (isNearWater ? "surprised" : "idle") : (isAscending ? "angelic" : (isDead ? "broken" : mood))))));
    ...
    return (
        <>
            <motion.div
                animate={isFiring ? { // Recoil effect
                ...
                style={{
                    ...
                    top: 0,
                    pointerEvents: 'none',
                    display: homePosition ? 'block' : 'none'
                }}
            >
                <motion.div
                    ...
                    onMouseEnter={() => {
                        setIsHovered(true);
                        recordInteraction();
                        if ((mood === 'bored' || mood === 'broken') && setMood) {
                            setMood('neutral');
                        }
                ...
                >
                    ...
                    <div style={{
                        ...
                        >
                            ...
                            {/* Sad Mood Effects (Outside core to avoid clipping) */}
                            <AnimatePresence>
                                {mood === 'sad' && (
                                    <>
                            ...
                            <AnimatePresence>
                                {isSmart && (
                                    <>
                                        ...
                                        {/* Neural Lines (Web-like connectors) */}
                                        <svg
                                            style={{
                                                position: 'absolute',
                                                width: '150%',
                            ...
                            {isSingularity ? (
                                ...
                                <>
                                    <motion.div
                                        className={`${styles.core} ${mood === 'sad' ? styles.sadCore : ''}`}
                                        id="nova-orb-core"
                                        ...
                                        style={{
                                            ...
                                            boxShadow: (isEgg && hatchPhase === 'hatched') ? 'none' : (isMelting
                                                ...
                                                : ((mood === 'broken' || isDead) ? 'none' : (mood === 'happy' ? '0 0 40px #fbbf24, inset 0 0 20px #fde047' : undefined))),
                                            opacity: (isEgg && hatchPhase === 'hatched') ? 0 : 1,
                                            pointerEvents: (isEgg && hatchPhase === 'hatched') ? 'none' : 'auto'
                                        }}
                                        variants={coreVariants}
                                        animate={gulpTrigger > 0 ? "gulp" : activeEmotion}
                                        initial={activeEmotion}
                                        transition={{ background: { duration: 0.8, type: "tween" } }}
                                    ...
                                    {/* Hatchling Chicken GIF - Moved here to avoid core clipping */}
                                    <AnimatePresence>
                                        {isEgg && hatchPhase === 'hatched' && (
                                            <motion.div
                                                initial={{ scale: 0, opacity: 0 }}
                                    ...
                                    <motion.div
                                        ...
                                        style={{
                                            x: eyeX,
                                            y: eyeY,
                                            borderRadius: mood === 'love' ? '0%' : '50%',
                                            background: mood === 'love' ? 'transparent' : colors.eye,
                                            boxShadow: isDead ? 'none' : undefined,
                                            opacity: (mood === 'broken' || mood === 'crazy' || isPacManMode || (isEgg && hatchPhase !== 'hatched')) ? 0 : 1,
                                            clipPath: mood === 'love' ? 'inset(0 0 10% 0)' : 'none'
                                        }}
                                        variants={eyeVariants}
                                        animate={isKillingMachine ? "killingMachine" : (isDead ? "broken" : (status === 'offline' || isPacManMode || mood === 'broken' || mood === 'crazy' ? "hidden" : (mood === 'love' ? 'love' : (activeEmotion === 'surprised' ? "surprised" : "idle"))))}
                                        initial="idle"
                                        transition={{
            ...
            {/* Separate Layer for Floating Hearts to avoid clipping */}
            <AnimatePresence>
                {mood === 'love' && (
                    <div style={{ position: 'absolute', inset: -100, pointerEvents: 'none', zIndex: 10000 }}>
                        {loveHearts.map(h => (
                            <motion.div
                                key={h.id}
                                style={{
                                ...
                                initial={{ opacity: 1, scale: 0.5, x: h.x, y: 0 }}
                                animate={{
                                    opacity: 0,
                                    scale: h.scale * 1.5,
                                    y: -200,
                                ...
                                transition={{ duration: 3, ease: "easeOut", delay: h.delay }}
                            >
                                <svg width="24" height="24" viewBox="0 0 24 24" style={{ filter: 'drop-shadow(0 0 5px #fb7185)' }}>
                                    <path
                                        d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41 0.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"
                                        fill="#fb7185"
                                    />
