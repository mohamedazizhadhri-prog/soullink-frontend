    ...
    useEffect(() => {
        ...
        const performSearchAndDestroy = async () => {
            ...
            if (foundNode && foundNode.parentElement) {
                ...
                setTimeout(() => {
                    setIsFiring(true);
                    // Calculate laser path from Nova's center
                    // Nova is usually 64x64, so center is position + 32
                    // But Nova moves to target.x - 32, target.y - 150
                    // So Nova's center is target.x, target.y - 150 + 32
                    const novaX = rect.left + rect.width / 2;
                    const novaY = rect.top + rect.height / 2 - 150 + 32;
